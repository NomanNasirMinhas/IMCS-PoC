...
export default ({ navigation, route }) => {
  ...
  const handleComplete = async (e) => {
    try {
      setProcessing(true);
      var res = await fetch(
        `https://crowdsensing-middleware.herokuapp.com/getResonses/${
          requestResult[selected - 1]._id
        }`
      );
      res = await res.json();
      if (res == false) {
        ...
        setProcessing(false)
        navigation.navigate("History", { responses: res });
      }
      setProcessing(false)
    } catch (e) {
      setProcessing(false);
    ...
    <View style={styles.container}>
      ...
      <ScrollView horizontal={false} showsVerticalScrollIndicator={true}>
        ...
        <Text style={[styles.text, { fontSize: 30 }]}>
          Please Select A Task To See Response
        </Text>
        <View style={[styles.container, { flexDirection: "column" }]}>
          <Input
            style={{ width: 100 }}
            label="Enter Task Number For Responses"
        ...
        <FlatList
          data={requestResult}
          renderItem={({ item }) => (
            <View style={[styles.container, { paddingTop:10, backgroundColor:'#EB984E', borderRadius:20, marginVertical:5 }]}>
              <Text style={styles.text}>Task No = {++counter}</Text>
              <Text style={styles.text}>Task Type = {item.taskType}</Text>
              <Text style={styles.text}>
                Task Location = {item.location.lat},{item.location.long}
              </Text>
              {!processing && (
                <Text style={styles.text}>
                  Distance ={" "}
